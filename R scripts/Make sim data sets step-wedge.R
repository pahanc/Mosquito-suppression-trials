...
#Define a function to make a simulated trial data set given the number of clusters per sequence
...
make_sim_counts<-function(cluster_per_group, G, runno){
  ...
  num_group=3 #Number of sequences
  num_period=3 #Number of trial periods, including a baseline year

  #load the posterior draws of mosquito counts for each cluster,compound,village,month and year
  load(paste("input files for cluster\\lp2_i_sw_",cluster_per_group,".RData",sep=""))  
  ...
  #Replicate period vector for each sequence
  ...
  #Add an offset to place clusters into sequences
  ...
  #Make a vector indicating whether each cluster receives the intervention in each period
  #according to the step-wedge design with a baseline year
  treatment<-c(rep('control',cluster_per_group),rep('intervention', 2*cluster_per_group), 
               rep('control', 2*cluster_per_group), rep('intervention', cluster_per_group), 
               rep('control', 3*cluster_per_group))
    ...
    for (grp in 1:num_group){
      for (cluster in 1:cluster_per_group){
        ...
        for (period1 in 1:num_period){
          if (period1 ==1){
            sel_cluster_ind<-sample(1:4,1)#select a cluster at random to represent each sequence
          ...
          for (mth in 1:12){
            ...
            for (samp in 1:length(samp_inds)){#Make data for each sample
              if (length(counts)>0){
